  ...
  unsigned char p_age, struct date p_entrydate)
{
  ...
  as->name = malloc(sizeof(char)*(1+strlen(p_name)));
  as->species = malloc(sizeof(char)*(1+strlen(p_species)));
  as->age = p_age;
...
{
  struct animal *elain = realloc(array, (size + 1)*sizeof(struct animal));
  init_animal(elain+size, newanimal.id, newanimal.name, newanimal.species,
  newanimal.age, newanimal.entrydate);
  return elain;
